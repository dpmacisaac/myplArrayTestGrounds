...
public class Parser {
  ...
  private void arrdecl_stmt() throws MyPLException{
    eat(TokenType.ARR, "expecting ARR in arrdecl_stmt");
    if(isPrimitiveType() || match(TokenType.ID)){
      dtype();
    }
    if(!match(TokenType.ASSIGN)){
      eat(TokenType.ID, "expecting ID in arrdecl_stmt");
    }
    eat(TokenType.ASSIGN, "expecting ASSIGN in arrdecl_stmt");
    eat(TokenType.LBRACE, "expecting LBRACE in arrdecl_stmt");
    args();
    eat(TokenType.RBRACE, "expecting RBRACE in arrdecl_stmt");
  }
